...
namespace SpellEditor.Sources.Controls
{
    public class SpellSelectionList : ListBox
    {
        ...
        {
            // Copy spell in DB
            using (var result = _Adapter.Query($"SELECT * FROM `{Config.Config.SpellTable}` WHERE `ID` = '{copyFrom}' LIMIT 1"))
            {
                var row = result.Rows[0];
                var str = new StringBuilder();
                str.Append($"INSERT INTO `{Config.Config.SpellTable}` VALUES ('{copyTo}'");
            ...
            using (var result = _Adapter.Query($"SELECT `id`,`SpellName{_Language - 1}`,`SpellIconID`,`SpellRank{_Language - 1}` FROM `{Config.Config.SpellTable}` WHERE `ID` = '{copyTo}' LIMIT 1"))
            {
        ...
        {
            // Delete from DB
            _Adapter.Execute($"DELETE FROM `{Config.Config.SpellTable}` WHERE `ID` = '{spellId}'");
        ...
        private DataRowCollection GetSpellNames(uint lowerBound, uint pageSize, int locale)
        {
            using (var newSpellNames = _Adapter.Query(
                string.Format(@"SELECT `id`,`SpellName{1}`,`SpellIconID`,`SpellRank{1}` FROM `{0}` ORDER BY `id` LIMIT {2}, {3}",
                 Config.Config.SpellTable, locale, lowerBound, pageSize)))
            {
